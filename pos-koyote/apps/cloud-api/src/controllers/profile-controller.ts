...
import { getProfile, updatePassword, updateProfile } from "../services/profile-service";
...
export async function updateProfileHandler(req: Request, res: Response) {
  ...
  try {
    ...
    const profile = await updateProfile(auth.userId, payload);
    res.status(200).json(profile);
  } catch (error) {
...
export async function updatePasswordHandler(req: Request, res: Response) {
  ...
  try {
    ...
    await updatePassword(auth.userId, payload.password);
